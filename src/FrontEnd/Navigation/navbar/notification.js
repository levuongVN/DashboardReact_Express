...
export default function NotificationDropdown() {
        ...
        if (email) {
            const FetchNotification = async () => {
                ...
                if (response.data.success) {
                    notificationRef.current = response.data.data;
    ...
    async function GetAcpStt(Stt, ProjectName, EmailPersonIsInvited) {
        try {
            ...
            if (response.data.success) {
                notificationRef.current = notificationRef.current.filter(item => item.ProjectName !== ProjectName);
                forceUpdate({}); // Trigger re-render
            }
            if (ws.current) {
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                {(notificationRef.current.length > 0 || isAccept.length > 0) && (
            ...
            {isOpen && (
                <div className="absolute -right-5 my-2 z-20 max-w-sm bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-800 dark:divide-gray-700">
                    ...
                    <div>
                        {notificationRef.current.map((item, index) => (
                                    ...
                                    <a href="#" className="">
                                        <div className="flex items-center rounded-lg py-1 text-black align-items-center px-4">
                                            ...
                                            <div className="ms-2 me-3">
                                                You were invited by "{item.EmailPersonInvited}" to "{item.ProjectName}" project
                                            </div>
                                        </div>
                                    </a>
